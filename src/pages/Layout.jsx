...
const Layout = ({ children }) => {
  ...
  const [error, setError] = useState(null); // Only for input error highlighting
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      ...
      try {
        ...
        const response = await axiosClient.get('/products/search', { params: { q: sanitizedQuery, limit: 5 } });
  ...
  // Temporary error setter (now uses global toast)
  const setTempError = useCallback((message, type = 'error') => {
    clearTimeout(errorTimeoutRef.current);
    setError(message);
    showToast(message, type, ERROR_TIMEOUT);
  ...
  const handleLogout = useCallback(async () => {
    try {
      ...
      setTempError('Failed to sign out. Please try again.', 'error');
    }
  ...
  const handleSearchSubmit = useCallback(
    async (e) => {
      ...
      if (!debouncedSearchQuery.trim()) {
        setTempError('Please enter a search term.', 'error');
        return;
      }
      setIsSearching(true);
      setIsSearchDropdownOpen(false);
      try {
        const sanitizedQuery = debouncedSearchQuery.trim().replace(/[<>]/g, '');
        const response = await axiosClient.get('/products/search', { params: { q: sanitizedQuery } });
        navigate('/search', {
