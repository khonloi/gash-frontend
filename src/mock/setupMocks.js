...
export const setupMocks = () => {
  ...
  const restrictedMessage =
    "This page is running in demo mode. To fully explore the project, please clone it and run it locally.";
  ...
  mock
    .onPut(/.*\/accounts\/change-profile\/.*/)
    .reply(400, { message: restrictedMessage });
  mock
    .onPut(/.*\/accounts\/change-password\/.*/)
    .reply(400, { message: restrictedMessage });
  mock
  ...
  mock.onGet(/.*\/(products|products)(\?.*)?$/).reply((config) => {
  ...
  mock.onGet(/.*\/(products|products)\/search(\?.*)?$/).reply((config) => {
    const urlParts = config.url.split("?");
    let filtered = [...mockProducts];
  ...
  mock.onGet(/.*\/(products|products)\/[a-zA-Z0-9_-]+/).reply((config) => {
  ...
  const initialProducts = [
    "6916c4f34945cd9a5d8631fb",
    "691836f7bc4af928d6320cb3",
  ];
  ...
  if (!currentOrders.find((o) => o._id === "demo_order_1")) {
    currentOrders.push({
      ...
      orderDetails: [
        {
          variantId: "6916c92382ece642cea141c1",
          quantity: 1,
          productPrice: 45,
        },
        {
          variantId: "69183e9eef9cc2d8dc359b3d",
          quantity: 1,
          productPrice: 80,
        },
      ],
  ...
  if (!currentOrders.find((o) => o._id === "demo_order_2")) {
    currentOrders.push({
      ...
      orderDetails: [
        {
          variantId: "69185569a17ac3f26c226e30",
          quantity: 2,
          productPrice: 55,
        },
      ],
  ...
  const enrichItem = (item) => {
    ...
    const vId = getID(
      item.variantId?._id || item.variantId || item.variant?._id,
    );
    ...
    if (!variant) {
      ...
      return {
        ...
        variantId: {
          _id: vId,
          productId: { productName: item.pro_name || "Unknown" },
        },
      };
    }
  ...
  mock.onGet(/.*\/carts\/account\/.*/).reply(handleGetCart);

  mock.onPost(/.*\/(carts|carts)/).reply((config) => {
    const data = JSON.parse(config.data);
    const cart = getStored(MOCK_KEYS.CART);
    const variantId = getID(data.variantId);

    const existingIndex = cart.findIndex(
      (item) => getID(item.variantId) === variantId,
    );
    ...
    if (existingIndex > -1) {
      const currentQty = parseInt(
        cart[existingIndex].quantity ||
          cart[existingIndex].productQuantity ||
          1,
        10,
      );
  ...
  mock.onPut(/.*\/(carts|carts)\/[a-zA-Z0-9_-]+/).reply((config) => {
    const parts = config.url.split("/");
    ...
    cart = cart.map((item) => {
      if (item._id === id) {
        ...
        if (data.productQuantity)
          updated.quantity = parseInt(data.productQuantity, 10);
        return updated;
      }
      return item;
    });
  ...
  mock.onDelete(/.*\/(carts|carts)\/[a-zA-Z0-9_-]+/).reply((config) => {
    const parts = config.url.split("/");
    const id = parts[parts.length - 1];
    let cart = getStored(MOCK_KEYS.CART);
      ...
      200,
      {
        data: favorites.map((item) => {
          const product = mockProducts.find(
            (p) => getID(p._id) === getID(item.productId),
          );
  ...
  mock.onGet(/.*\/order-details\/search(\?.*)?$/).reply((config) => {
    ...
    allOrders.forEach((order) => {
      const enrichedItems = (order.orderDetails || order.items || []).map(
        (item) => ({
          ...enrichItem(item),
          orderId: order._id,
      ...
      if (q) {
        allItems = allItems.filter(
          (item) =>
            ...
            (item.variantId &&
              item.variantId.productColorId &&
              item.variantId.productColorId.productColorName &&
              item.variantId.productColorId.productColorName
                .toLowerCase()
                .includes(q)),
        );
  ...
  mock.onAny().reply((config) => {
    console.warn(
      `[GASH Demo] Unmatched Request: ${config.method.toUpperCase()} ${config.url}`,
      config,
    );
