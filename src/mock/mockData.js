...
const imagesMap = Object.fromEntries(
  imagesRaw.map((i) => [getID(i._id), { ...i, _id: getID(i._id) }]),
);
...
export const mockUser = {
  ...
  role: "customer",
  image:
    "https://static.wikia.nocookie.net/gtawiki/images/a/ae/TommyVercetti-GTAVC.jpg",
  gender: "male",
  dob: "1951-08-28",
...
export const mockProducts = productsRaw.map((p) => {
  ...
  return {
    ...
    productImageIds: (p.productImageIds || [])
      .map((id) => imagesMap[getID(id)])
      .filter(Boolean),
    productVariantIds: (p.productVariantIds || [])
      .map((id) => variantsMap[getID(id)])
      .filter(Boolean),
